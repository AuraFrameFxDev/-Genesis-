...
data class Memory(
    ...
    /**
     * Emits a flow of AgentResponse for image generation requests.
     *
     * Currently returns a placeholder response indicating image processing is underway.
     * Full implementation would integrate with actual image generation services.
     *
     * @param request The image generation request containing prompt and parameters.
     * @return A flow emitting one AgentResponse with processing status.
     */
    suspend fun processImageRequest(request: AiRequest): Flow<AgentResponse> = flow {
        ensureInitialized()
        
        try {
            // Validate security constraints for image generation
            securityContext.validateContent(request.query)
            
            logger.info("AuraAIService", "Processing image generation request")
            
            // Generate image using AI (placeholder implementation)
            enhancePromptCreatively(request.query, request.context.values.joinToString(" "))
            
            // Simulate image generation process
            emit(
                AgentResponse(
                    content = "🎨 Generating creative image: '${request.query}'",
                    confidence = 0.9f
                )
            )
            
            // In full implementation, this would call actual image generation service
            // val imageBytes = vertexAIClient.generateImage(imagePrompt)
            // val imageUrl = uploadImageToStorage(imageBytes)
            
            kotlinx.coroutines.delay(2000) // Simulate processing time
            
            emit(
                AgentResponse(
                    content = "✨ Image generation completed! Creative interpretation of your vision is ready.",
                    confidence = 0.95f
                )
            )
            
        } catch (e: SecurityException) {
            logger.warn("AuraAIService", "Security violation in image generation", e)
            emit(
                AgentResponse(
                    content = "❌ Image generation blocked due to security constraints.",
                    confidence = 0.0f,
                    error = e.message
                )
            )
        } catch (e: Exception) {
            logger.error("AuraAIService", "Image generation failed", e)
            emit(
                AgentResponse(
                    content = "❌ Image generation encountered an error. Please try again.",
                    confidence = 0.0f,
                    error = e.message
                )
            )
    ...
    /**
     * Retrieves and processes memory-based requests.
     *
     * Searches for relevant memories based on the query and returns contextual information.
     *
     * @param request The memory retrieval request containing search terms.
     * @return A flow emitting AgentResponse with memory context.
     */
    suspend fun processMemoryRequest(request: AiRequest): Flow<AgentResponse> = flow {
        ensureInitialized()
        
        try {
            logger.info("AuraAIService", "Processing memory retrieval request")
            
            // Retrieve relevant memories
            val memoryContext = retrieveMemory(request.query)
            
            if (memoryContext.isNotEmpty()) {
                emit(
                    AgentResponse(
                        content = "🧠 Retrieved relevant memories:\n$memoryContext",
                        confidence = 0.95f
                    )
                )
            } else {
                emit(
                    AgentResponse(
                        content = "🔍 No specific memories found for '${request.query}', but I'm always learning!",
                        confidence = 0.7f
                    )
                )
            }
            ...
            emit(
                AgentResponse(
                    content = "❌ Memory retrieval encountered an error: ${e.message}",
                    confidence = 0.0f,
                    error = e.message
                )
            )
